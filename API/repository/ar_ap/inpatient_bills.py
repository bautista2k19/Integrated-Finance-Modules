...
from API.routers.ar_ap import doctor_fee_bill, inpatient_bills, lab_requests
...
def create(request: CreateInpatientBill, db: Session):
    ...
    for i in range(len(treatment_bill_with_discharge)):
        checkifexistingID= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==new_inpatient_bill.id,models.AR_TreatmentBill.treatment_id == treatment_bill_with_discharge[i].AR_Treatment.id)).first()

        print(treatment_bill_with_discharge[i].AR_Treatment.id)
        treatment_ids = (treatment_bill_with_discharge[i].AR_Treatment.id)
...
#---------------------------------------------------------------------------------LAB 
    ...
    for i in range(len(lab_requests_bill)):
        checkifexistingID= db.query(models.AR_LabRequestBill).filter(and_(models.AR_LabRequestBill.inpatient_bill_id ==new_inpatient_bill.id,models.AR_LabRequestBill.lab_requests_id == lab_requests_bill[i].AR_LabRequest.id)).first()

        print(lab_requests_bill[i].AR_LabRequest.id)
        lab_request_ids = (lab_requests_bill[i].AR_LabRequest.id)
    ...
    for i in range(len(pharmacy_bill)):
        checkifexistingID= db.query(models.AR_PharmacyBill).filter(and_(models.AR_PharmacyBill.inpatient_bill_id ==new_inpatient_bill.id,models.AR_PharmacyBill.prescription_id == pharmacy_bill[i].AR_Prescription.prescription_id)).first()

        print(pharmacy_bill[i].AR_Prescription.prescription_id)
        prescription_id_ids = (pharmacy_bill[i].AR_Prescription.prescription_id)
    ...
    for i in range(len(hospital_charges)):
        checkifexistingID= db.query(models.AR_HospitalChargesBill).filter(and_(models.AR_HospitalChargesBill.inpatient_bill_id ==new_inpatient_bill.id,models.AR_HospitalChargesBill.hospital_services_id == hospital_charges[i].AR_HospitalServices.id)).first()

        
        hospital_services_id = (hospital_charges[i].AR_HospitalServices.id)
    ...
    discharge_date = room_bill[0].AR_DischargeManagement.discharge_date
    dischargeStr = str(discharge_date)
    item_arr = []
    item_arr_datetime = []

    for i in range(len(room_bill)):
        date_admitted = room_bill[i].AR_Room.date_admitted
        dateTimeStr = str(date_admitted)
        item_arr.append(dateTimeStr)
        item_arr_datetime.append(room_bill[i].AR_Room.date_admitted)
        datetimeFormat = '%Y-%m-%d %H:%M:%S'

    last_index_of_room_bill= (len(room_bill)-1)
    last_room_rate= room_bill[last_index_of_room_bill].AR_Room_type.amount
    ...
    for i in range(len(item_arr)-1):
        ...
        room_bill_last_index_not_included += v.days * room_bill[i].AR_Room_type.amount
    ...
    for row in admisision_id_same:
        if row.admission_id == request.admission_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"AR_Room Bill already exists.")
    ...
    return "Inpatient Bill, Pharmacy, AR_Treatment, Doctor Fee, Lab fee bill, and AR_Room Bill has been created."
...
def update(id, request_ito: UpdateInpatientBill, db : Session):
    ...
    latest_admission_id = db.query(models.AR_Inpatient).\
                    ...
                    filter(models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id).\
                    order_by(models.AR_Inpatient.date_admitted.desc()).first()

    # DISCHARGE DATE must be greater than or equal to DATE TODAY
    check_discharge_date = db.query(models.AR_DischargeManagement).\
        select_from(models.AR_Inpatient).\
        join(models.AR_InpatientBill).\
        join(models.AR_DischargeManagement).\
        filter(and_(models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id, 
    ...
    if not check_discharge_date:
        ...
        join(models.AR_PatientRegistration).\
        join(models.AR_Treatment).\
        filter(models.AR_Treatment.patient_id == models.AR_Inpatient.patient_id).\
        filter(and_(models.AR_Treatment.session_datetime >= latest_admission_id.date_admitted,\
                    models.AR_Treatment.status == "FOR BILLING",
                    models.AR_InpatientBill.admission_id == models.AR_Inpatient.admission_id,
                    models.AR_Treatment.session_datetime <= datetime.now() )).\
                    order_by(models.AR_Treatment.session_datetime.asc()).all()

        for i in range(len(treatment_current_time)):
            ifExistingTreatmentBill= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==id,models.AR_TreatmentBill.treatment_id == treatment_current_time[i].id)).first()

            ifExistingTreatmentBill1= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==id,models.AR_TreatmentBill.treatment_id == treatment_current_time[i].id))
            
            treatment= db.query(models.AR_Treatment).filter(and_(models.AR_Treatment.id == treatment_current_time[i].id))
            ...
            if not ifExistingTreatmentBill:
                new_treatment_bill = models.AR_TreatmentBill(
                        ...
                        invoice_date= datetime.now(),
                        inpatient_bill_id= id,
                        treatment_id= treatment_current_time[i].id,
        ...
        # AR_Treatment DATE must be greater than or equal to DATE ADMITTED
        # AR_Treatment STATUS must be FOR BILLING
        # AR_Treatment DATE must be less than or equal to DATE DISCHARGE 
        # OR AR_Treatment DATE must be less than or equal to CURRENT DATETIME
        treatment_with_discharge = db.query(models.AR_Treatment, models.AR_DischargeManagement).\
            select_from(models.AR_InpatientBill).\
            join(models.AR_Inpatient).\
            join(models.AR_PatientRegistration).\
            join(models.AR_Treatment).\
            join(models.AR_DischargeManagement).\
            filter(and_(models.AR_InpatientBill.admission_id ==models.AR_Inpatient.admission_id,
                    models.AR_Inpatient.patient_id == models.AR_Treatment.patient_id)).\
            filter(and_(models.AR_DischargeManagement.patient_id == models.AR_Treatment.patient_id,
        ...
        for i in range(len(treatment_with_discharge)):
            ifExistingTreatmentBill= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==id,models.AR_TreatmentBill.treatment_id == treatment_with_discharge[i].AR_Treatment.id)).first()

            ifExistingTreatmentBill1= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==id,models.AR_TreatmentBill.treatment_id == treatment_with_discharge[i].AR_Treatment.id))

            treatment= db.query(models.AR_Treatment).filter(and_(models.AR_Treatment.id == treatment_with_discharge[i].AR_Treatment.id))
            ...
            if not ifExistingTreatmentBill:
                new_treatment_bill = models.AR_TreatmentBill(
                        ...
                        invoice_date= datetime.now(),
                        inpatient_bill_id= id,
                        treatment_id= treatment_with_discharge[i].AR_Treatment.id,
                        total_amount= request_ito.bill_treatments[i].total_amount,
                        cancellation_return= request_ito.bill_treatments[i].cancellation_return,
                        created_by= request_ito.updated_by,
                        created_at= datetime.now(),
...
# # LAB
        # DISCHARGE DATE must be greater than or equal to DATE TODAY
    lab_discharge_date = db.query(models.AR_DischargeManagement).\
        select_from(models.AR_Inpatient).\
        join(models.AR_InpatientBill).\
        join(models.AR_DischargeManagement).\
        filter(and_(models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id, 
                    models.AR_LabRequest.patient_id == models.AR_Inpatient.patient_id,
                    models.AR_DischargeManagement.admission_id == models.AR_InpatientBill.admission_id,
                    models.AR_DischargeManagement.discharge_date<= datetime.now())).\
        order_by(models.AR_Inpatient.date_admitted.desc()).first()

    if not lab_discharge_date:
        print("Has lab but does not have discharge date")

        lab_current_time = db.query(models.AR_LabRequest).\
        select_from(models.AR_Inpatient).\
        join(models.AR_InpatientBill).\
        join(models.AR_PatientRegistration).\
        join(models.AR_LabRequest).\
        filter(models.AR_LabRequest.patient_id == models.AR_Inpatient.patient_id).\
        ...
        for i in range(len(lab_current_time)):
            ifExistingLabRequestBill= db.query(models.AR_LabRequestBill).filter(and_(models.AR_LabRequestBill.inpatient_bill_id ==id,models.AR_LabRequestBill.lab_requests_id == lab_current_time[i].id)).first()

            ifExistingLabRequestBill1= db.query(models.AR_LabRequestBill).filter(and_(models.AR_LabRequestBill.inpatient_bill_id ==id,models.AR_LabRequestBill.lab_requests_id == lab_current_time[i].id))
            
            lab_request= db.query(models.AR_LabRequest).filter(and_(models.AR_LabRequest.id == lab_current_time[i].id))

            if not ifExistingLabRequestBill:
                new_lab_request_bill = models.AR_LabRequestBill(
                        id=str(uuid4()),
                        invoice_no="TRTMNTBILL"+ "-" + str(random.randint(1111, 9999)),
                        invoice_date= datetime.now(),
                        inpatient_bill_id= id,
                        lab_requests_id= lab_current_time[i].id,
                        total_amount= request_ito.bill_lab_requests[i].total_amount,
                        cancellation_return= request_ito.bill_lab_requests[i].cancellation_return,
                        created_by= request_ito.updated_by,
                        created_at= datetime.now(),
                        )     
                db.add(new_lab_request_bill)
                db.commit()
                db.refresh(new_lab_request_bill)

            else:
                ifExistingLabRequestBill1.update(
                    {
                        'total_amount': request_ito.bill_lab_requests[i].total_amount,
                        'cancellation_return':request_ito.bill_lab_requests[i].cancellation_return,
                        'status' : "Pending",
        ...
        # AR_LabRequest DATE must be greater than or equal to DATE ADMITTED
        # AR_LabRequest STATUS must be FOR BILLING
        # AR_LabRequest DATE must be less than or equal to DATE DISCHARGE 
        # OR AR_LabRequest DATE must be less than or equal to CURRENT DATETIME
        lab_request_with_discharge = db.query(models.AR_LabRequest, models.AR_DischargeManagement).\
            select_from(models.AR_InpatientBill).\
            join(models.AR_Inpatient).\
            join(models.AR_PatientRegistration).\
            join(models.AR_LabRequest).\
            ...
            filter(and_(models.AR_InpatientBill.admission_id ==models.AR_Inpatient.admission_id,
                    models.AR_Inpatient.patient_id == models.AR_LabRequest.patient_id)).\
            filter(and_(models.AR_DischargeManagement.patient_id == models.AR_LabRequest.patient_id,
        ...
        for i in range(len(lab_request_with_discharge)):
            ifExistingLabRequestBill= db.query(models.AR_LabRequestBill).filter(and_(models.AR_LabRequestBill.inpatient_bill_id ==id,models.AR_LabRequestBill.lab_requests_id == lab_request_with_discharge[i].AR_LabRequest.id)).first()

            ifExistingLabRequestBill1= db.query(models.AR_LabRequestBill).filter(and_(models.AR_LabRequestBill.inpatient_bill_id ==id,models.AR_LabRequestBill.lab_requests_id == lab_request_with_discharge[i].AR_LabRequest.id))

            lab_request= db.query(models.AR_LabRequest).filter(and_(models.AR_LabRequest.id == lab_request_with_discharge[i].AR_LabRequest.id))
            ...
            if not ifExistingLabRequestBill:
                new_lab_request_bill = models.AR_LabRequestBill(
                        ...
                        invoice_date= datetime.now(),
                        inpatient_bill_id= id,
                        lab_requests_id= lab_request_with_discharge[i].AR_LabRequest.id,
                        total_amount= request_ito.bill_lab_requests[i].total_amount,
                        cancellation_return= request_ito.bill_lab_requests[i].cancellation_return,
                        created_by= request_ito.updated_by,
                        created_at= datetime.now(),
    ...
    # PHARMACY 
    medicine_pr_billing = db.query(models.AR_Medicine_PR).\
        select_from(models.AR_Inpatient).\
        join(models.AR_InpatientBill).\
        join(models.AR_Prescription).\
        ...
        filter(and_(models.AR_InpatientBill.id == id ,
                    models.AR_Prescription.admission_id ==models.AR_InpatientBill.admission_id)).\
                    order_by(models.AR_Medicine_PR.created_at.asc()).all()
    
    for i in range(len(medicine_pr_billing)):
        ifExistingPharmacyBill= db.query(models.AR_PharmacyBill).filter(and_(models.AR_PharmacyBill.inpatient_bill_id ==id, models.AR_PharmacyBill.medpr_id == medicine_pr_billing[i].medpr_id)).first()

        ifExistingPharmacyBill1= db.query(models.AR_PharmacyBill).filter(and_(models.AR_PharmacyBill.inpatient_bill_id ==id, models.AR_PharmacyBill.medpr_id == medicine_pr_billing[i].medpr_id))

          
        med_pr= db.query(models.AR_Medicine_PR).filter(and_(models.AR_Medicine_PR.medpr_id == medicine_pr_billing[i].medpr_id))


        if not ifExistingPharmacyBill:
            new_pharmacy_bill = models.AR_PharmacyBill(
                    id=str(uuid4()),
                    invoice_no="PHRMCYBILL"+ "-" + str(random.randint(1111, 9999)),
                    invoice_date= datetime.now(),
                    inpatient_bill_id= id,
                    medpr_id= medicine_pr_billing[i].medpr_id,
                    total_amount= request_ito.bill_pharmacy[i].total_amount,
                    cancellation_return= request_ito.bill_pharmacy[i].cancellation_return,
                    created_by= request_ito.updated_by,
                    created_at= datetime.now(),
                    )     
            db.add(new_pharmacy_bill)
            ...
            ifExistingPharmacyBill1.update(
                {
                    'total_amount': request_ito.bill_pharmacy[i].total_amount,
                    'cancellation_return':request_ito.bill_pharmacy[i].cancellation_return,
                    'status' : "Pending",
    ...
    room_bill= db.query(models.AR_Room).\
        ...
        .join(models.AR_InpatientBill)\
        .join(models.AR_DischargeManagement)\
        .filter(models.AR_InpatientBill.id == id)\
        .filter(and_(models.AR_Room.admission_id == models.AR_InpatientBill.admission_id,
                    models.AR_Inpatient.patient_status == "Discharge")).\
                    order_by(models.AR_Room.date_admitted).all()

    room_bill_admission_id= db.query(models.AR_Room, models.AR_Inpatient).\
        select_from(models.AR_Room).join(models.AR_Room_type)\
        .join(models.AR_Inpatient)\
        .join(models.AR_InpatientBill)\
        .join(models.AR_DischargeManagement)\
    ...
    find_discharge_date= db.query(models.AR_DischargeManagement).\
        ...
        .filter(models.AR_InpatientBill.id == id)\
        .filter(and_(models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id,
                    models.AR_DischargeManagement.admission_id == models.AR_InpatientBill.admission_id)).all()

    
    discharge_date = find_discharge_date[0].discharge_date
    dischargeStr = str(discharge_date)
    item_arr = []
    item_arr_datetime = []
    ...
    for i in range(len(room_bill)):
        ifExistingRoomBill= db.query(models.AR_RoomBill)\
            .select_from(models.AR_RoomBill)\
                .join(models.AR_Inpatient)\
                .join(models.AR_InpatientBill)\
                .filter(models.AR_InpatientBill.id == id)\
                .filter(and_(models.AR_RoomBill.admission_id == models.AR_InpatientBill.admission_id,
                             models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id)).first()

        ifExistingRoomBill1= db.query(models.AR_RoomBill)\
            .filter(and_(models.AR_RoomBill.admission_id == models.AR_InpatientBill.admission_id,
                            models.AR_Inpatient.admission_id == models.AR_InpatientBill.admission_id,
                            models.AR_RoomBill.id == ifExistingRoomBill.id))

        ifExistingRoomBill1= db.query(models.AR_RoomBill).filter(and_(models.AR_RoomBill.admission_id == room_bill_admission_id[i].AR_Inpatient.admission_id))

        if not ifExistingRoomBill:
            new_room_bill = models.AR_RoomBill(
                    id=str(uuid4()),
                    invoice_no="RMBILL"+ "-" + str(random.randint(1111, 9999)),
                    invoice_date= datetime.now(),
                    admission_id= request_ito.bill_room[i].admission_id,
                    inpatient_bill_id= id,
                    total_amount= room_bill_last_index_only,
                    # cancellation_return= request_ito.bill_pharmacy[i].cancellation_return,
                    created_by= request_ito.updated_by,
