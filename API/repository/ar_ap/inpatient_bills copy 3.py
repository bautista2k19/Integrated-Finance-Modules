...
def create(request: CreateInpatientBill, db: Session):
    ...
    for i in range(len(room_bill)):
        checkifexistingID= db.query(models.AR_Room).\
                        filter(and_(models.AR_Room.admission_id == request.admission_id
                                ,models.AR_Room.room_id == room_bill[i].room_id)).first()
                       
    return checkifexistingID    


    # room_bill= db.query(models.AR_Room).select_from(models.AR_Room).join(models.AR_Inpatient).\
    #                     filter(and_(models.AR_Room.admission_id == request.admission_id,
    #                                 models.AR_Inpatient.patient_status == "Discharge")).all()
    # return room_bill
...
#         date_of_billing= datetime.datetime.now(),
...
#         print(hospital_charges[i].HospitalServices.id)
...
#     return "Inpatient Bill, Pharmacy, Treatment, Doctor, and Lab fee bill has been created."
