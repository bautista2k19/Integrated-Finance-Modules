...
def update(id, request_ito: UpdateInpatientBill, db : Session):
    ...
    # eto otry mo inpatient biill talaga na id
    ...
    treatment_bill_with_discharge= db.query(models.AR_Inpatient, models.AR_Treatment).\
            select_from(models.AR_Inpatient).\
                    ...
                    filter(and_(models.AR_Inpatient.admission_id == request_ito.admission_id,
                                models.AR_Inpatient.patient_id == models.AR_Treatment.patient_id)).all()
                                
    #hini yan
    #same number lang ba tong treatment bill with discharge #tska request?oo join pa?
    #same parin to e hahahah
    #paano ba malalaman pag hindi siya nag eexist
    #paano mo pinapasanm
    #paano mo pinapasa tong value
    for i in range(len(treatment_bill_with_discharge)-1):
        print(f"index number:{i}")
        ifExistingTreatmentBill= db.query(models.AR_TreatmentBill).filter(and_(models.AR_TreatmentBill.inpatient_bill_id ==id,models.AR_TreatmentBill.treatment_id == treatment_bill_with_discharge[i].Treatment.id, models.AR_Treatment.status == "FOR BILLING")).first()
#parang lagi naman tong nag eexist e hahahah natry ko yan e pag wala nadadagdag
        ...
        #gawa ka nga bagong inpatient hahahahah takte nawala kasi e kala ko pwed eidelete yun e hahahah
        
        #bat nawala hahah yung sa ui sa inpatient bill nakadepende yun
#gumana ba o hindi nasobrahan pag for billing lang yun status ayun nalng ilagay mo haha
        # ang gulo neto HAHAHAAH prang lagi naman siya mag eexist e hahahah
        #lagay ka nga ng di nageexist dito
        treatment_ids = (treatment_bill_with_discharge[i].Treatment.id)
        if not ifExistingTreatmentBill:
            ...
            #anyare
            #san mo ginawa
            #may mali pa?oo di nadagdag yung bagong treatment pag if not exist madadagdag yun e
            # Ihiwalay ko nalnag ba haha
               
                print(f"total amount :{request_ito.bill_treatments[i].total_amount}")
                print(f"cancellation_return':{request_ito.bill_treatments[i].cancellation_return}")
                #ayan kaya nasasabay sabay siya mag update kasi na all na 
                # update all treatment bill na hindi na by id kasi nalabas na e so dapat
                # hindi nagdidiresto amp sa 0 index lang e 3 pinasa mong request e
                #guamana pala may mali lang index
                ifExistingTreatmentBill1.update(
                    {
                        # 'id': request_ito.bill_treatments[y].id,
                        #ayan?hindi nadoble
                        # 9 siya ilan lang ba dapat?3 isee
                        # bale ganto ata gingawa niya
                        'total_amount': request_ito.bill_treatments[i].total_amount,
                        'cancellation_return':request_ito.bill_treatments[i].cancellation_return,
                        'status' : "Pending",
                        'updated_by': request_ito.updated_by,
                ...
                #bhaliw amphahahahahhahaha #testing mo ule gawa ka bago baka magka eror
