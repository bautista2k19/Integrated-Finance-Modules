...
def find_all_for_billing(id, db: Session):
    prescription = db.query(models.AR_Prescription,models.AR_InpatientBill, models.AR_Inpatient).\
        select_from(models.AR_Inpatient).\
        join(models.AR_InpatientBill).\
        join(models.AR_Prescription).\
        filter(and_(models.AR_InpatientBill.admission_id == id == models.AR_Prescription.admission_id == id ==models.AR_InpatientBill.admission_id == id)).first()
    ...
    check_discharge_date = db.query(models.AR_DischargeManagement).\
        ...
        join(models.AR_Prescription).\
        filter(and_(models.AR_Prescription.admission_id == id == models.AR_DischargeManagement.admission_id == id)).\
        first()
...
# # OR Treatment DATE must be less than or equal to CURRENT DATETIME
    if not check_discharge_date:
        ...
        if not medicine_pr_with_discharge_date:  
