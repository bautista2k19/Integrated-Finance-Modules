from API.schemas.ar_ap.surgery_bill import CreateSurgeryBill, ShowSurgeryBill, UpdateSurgeryBill
...
from API.repository.ar_ap import surgery_bill

# from AR_AP.repository.surgery_bill import surgery_bill

router = APIRouter(
    prefix="/AR_AP/user/surgery_bill",
    tags=['Surgery Bill']
)
...
# @router.get('/find_by_name/{term_name}', status_code=status.HTTP_200_OK, response_model=ShowSurgeryBill)
# def find_by_term_name(term_name:str, db: Session = Depends(get_db)):#, current_user: User = Depends(oauth2.get_current_user)):
#     return surgery_bill.find_by_term_name(term_name, db)

@router.post('/', status_code=status.HTTP_201_CREATED)
def create(
    request: CreateSurgeryBill, 
    db: Session = Depends(get_db),
    result = Depends(security.auth)
):#, current_user: User = Depends(oauth2.get_current_user)):
    try:
        ...
        return surgery_bill.create(request, db)


@router.put('/{id}', status_code=status.HTTP_202_ACCEPTED)
def update(
    id, 
    request: UpdateSurgeryBill, 
    db: Session = Depends(get_db),
    result = Depends(security.auth)
):#, current_user: User = Depends(oauth2.get_current_user)):
    try:
        ...
        return surgery_bill.update(id, request, db)
...
def completed(
    ...
    try:
        ...
        return surgery_bill.completed(id,updated_by,db)