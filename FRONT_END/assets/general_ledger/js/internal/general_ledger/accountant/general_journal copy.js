            ...
            const action = 
              `<div class="text-center dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right p-0" aria-labelledby="dropdownMenuLink">
                  ...
                  <a class="dropdown-item " onclick="getData('${id}', 0);"><i class="fas fa-edit mr-1" style="width: 2rem;"></i><span>Edit</span></a>
                  ${
...
const loadAccountsViewTable = (nthTable, accounts, input) => {
  ...
  accounts.forEach((account) => {
    ...
    if ( hasAppliedInterest ) {
      ...
      totalAmount = parseFloat(
        ((account.credit * (account.rate/100) * (account.month_no/12)) + account.credit)
          .toLocaleString('en', {'maximumFractionDigits': 2, 'useGrouping': false})
      );
    }
...
const forEditOrViewCallback = (responseData) => {console.log(responseData)
  ...
  //For all entry types
    ...
    else if ( entry.je_originating_entry != undefined ) accessType = -2;
  }
        ...
        <dl class="row">
          ...
          <div class="col-12">
            <div class="row">
              ...
              <dd class="col-sm-4">${
                //journalized_by
                formatName(entry.je_journalized_by)
              }</dd>
            </div>
          </div>
          ${ //POSTED
          ...
          ${ //UPDATED
            ...
            ? `<div class="col-12">
                <div class="row">
                  ...
                  <dd class="col-sm-4">${
                    //updated_by
                    formatName(entry.je_updated_by)
                  }</dd>
                </div>
              </div>`
            : ``
            ...
            <dl class="row">
              <div class="col-12">
                <div class="row">
                  ...
                  <dd class="col-sm-4">${
                    //journalized_by
                    formatName(data.je_journalized_by)
...
const prepareAndSendData = (formData) => {
  ...
  if ( originating_entry.id != undefined ) {
    ...
    if ( /^(Depreciation|P \* R \* T)$/i.test(method) ) {
      ...
      const isAdjustedAccountAdjustable = balance == totalAmount ? false : true;
      if ( !isAdjustedAccountAdjustable ) accountToAdjust.delete(adjusted_account);

      //account
      originating_entry['account'] = {
        'balance': balance,
        'is_adjustable': isAdjustedAccountAdjustable
      };
    //Calculation for asset & liability
      ...
      if ( adjusted_balance <= balance ) {
        ...
        const isAdjustedAccountAdjustable = balance == 0 ? false : true;
        if ( !isAdjustedAccountAdjustable ) accountToAdjust.delete(adjusted_account);
    ...
    originating_entry['is_adjustable'] = accountToAdjust.size == 0 ? false : true;
  ...
  for (const row of rows) {
    ...
    //Ensure the depreciation / credit liability account is adjustable
    //balance must be 0
    if ( 
      ( account['salvage_value'] > 0 && account['useful_life'] > 0 )
      || ( account['rate'] > 0 && account['month_no'] > 0 )
    ) {
